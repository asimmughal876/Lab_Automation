...
if(isset($_POST['submit']))
{
  ...
  $query = "SELECT * FROM `cpri` WHERE cpri_email = '$email' AND cpri_password = '$pass'";
  $run = mysqli_query($conn, $query);
  $row = mysqli_fetch_assoc($run);
  if(mysqli_num_rows($run)>0)
  {
 
  $_SESSION['User_Id'] = $row['cpri_id'];
  $_SESSION['User_Name'] = $row['cpri_name'];
  $_SESSION['User_Email'] = $row['cpri_email'];
  $_SESSION['User_Pass'] = $row['cpri_password'];
